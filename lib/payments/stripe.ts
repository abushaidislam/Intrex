...
export async function createCustomerPortalSession(tenant: Tenant) {
  if (!tenant.stripeCustomerId || !tenant.stripeProductId) {
    redirect('/pricing');
  }
  ...
  if (configurations.data.length > 0) {
    ...
    const product = await stripe.products.retrieve(tenant.stripeProductId);
    if (!product.active) {
      throw new Error("Tenant's product is not active in Stripe");
    }
    ...
    if (prices.data.length === 0) {
      throw new Error("No active prices found for the tenant's product");
    }
